...
# read file inmkl
from utils.read_inmkl import read_inkml as parseINKML
...
class AuthorHWDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        #my_images_path =os.listdir("data/data_IAM")  # images IAM dataset
        my_images_path ="data/vn_handwriting_data/images/"
        ...
        # for page_idx, name in enumerate(my_images_path):
        #     lines,author = parseXML(os.path.join(dirPath,'xml',name.replace(".png","")+'.xml'))
        #     self.author_list.add(author)

        path_viet_inkml = "data/vn_handwriting_data/InkData_line/"
        for s in os.listdir(path_viet_inkml):
            lines,author = parseINKML(s, path_viet_inkml, my_images_path)

            authorLines = len(self.authors[author])
            #self.authors[author] += [(os.path.join(dirPath,"data_IAM",name),)+l for l in lines]
            self.authors[author] += [(os.path.join(path_viet_inkml,s),)+l for l in lines]
            self.author_list.add(author)
            ...
            for author,lines in self.lineIndex:
                for line in lines:
                    ...
                    if not os.path.exists(fg_path):
                        img = cv2.imread(lb,0) #read as grayscale, crop line
                        img= self.remove_whitespace(img,127)
    ...
    def remove_whitespace(self, img, thresh, remove_middle=False):
    ...
    def __getitem__(self, idx):
        ...
        for author,line in alines:
            ...
            if type(self.augmentation) is str and 'normalization' in  self.augmentation and self.normalized_dir is not None and os.path.exists(os.path.join(self.normalized_dir,'{}_{}.png'.format(author,line))):
                ...
                img = cv2.imread(lb,0)
                if img is None:
                    print('Error, could not read image: {}'.format(lb))
                    return None
